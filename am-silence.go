...
// Matcher : containts Alertmanager matcher data
type Matcher struct {
...
// ReceivedSilences : containts Alertmanager request silence data
...
func genSilence(silencePeriod int, labels, creator, comment string) Silence {
	silence := Silence{
		StartsAt: time.Now().UTC().Format(time.RFC3339),
		EndsAt: time.Now().Add(time.Second * time.Duration(silencePeriod)).UTC().Format(time.RFC3339),
		Comment: comment,
		CreatedBy: creator,
...
func addSilence(amURL string, timeout, silencePeriod int, labels, creator, comment string) error {
	silence := genSilence(silencePeriod, labels, creator, comment)

	fmt.Printf("Adding silence [creator: %s, comment: %s, start: %s, end: %s]\n", silence.CreatedBy, silence.Comment, silence.StartsAt, silence.EndsAt)
...
func main() {
	...
	if err := process(*amURL, *timeout, *mode, *labels, *silencePeriod, *creator, *comment); err != nil {
